...
describe "get new_customer_path" do
  ...
  describe "get edit_customer_path" do
    it "renders the :edit template" do
      customer=FactoryBot.create(:customer)
      get edit_customer_path(customer.id), params: {customer: {first_name: "Iggy"}}
  ...
  describe "put customer_path with valid data" do
    it "updates an entry and redirects to the show path for the customer" do
      customer=FactoryBot.create(:customer)
      put customer_path(customer.id), params: {customer: {first_name: "Iggy"}}
      customer.reload
      expect(customer.first_name).to eq("Iggy")
  ...
  describe "put customer_path with invalid data" do
    it "does not update the customer record or redirect" do
      customer=FactoryBot.create(:customer)
      put customer_path(customer.id), params: {customer: {phone: "123456789"}}
      customer.reload
      expect(customer.phone).to_not eq("123456789")
      expect(response.status).to eq(200)
    end
  end
